...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Step 4: Selective download — only the missing task folders
        allow_patterns = [f"{s}/{t}/**" for (s, t) in missing]
        print(f"Downloading {len(missing)} task folders from {SOURCE_REPO}...")
        snapshot_download(
            repo_id=SOURCE_REPO,
            repo_type="dataset",
